...
api_ctrl_without_sensor_data_t auto_tasks_send_data = {0,0,0,0,0};  //auto tasks send data
bool isAutoTasks = true;
int16_t rcMode = 0;  //rcMode:P A F
...
#if 1
	//use transmitter mode Button to start auto tasks
	rcMode = recv_sdk_std_msgs.rc.mode;
	if(rcMode == 8000 && isAutoTasks)
	{
		isAutoTasks = false;
		/* api activation */
		...
		sleep(8);
		/* auto tasks */
		DJI_Onboard_API_Start_Auto_Tasks();
		sleep(3);
		/* go home */
...
//auto tasks send data function
void auto_tasks_send_data_fun(char ctrl, float roll, float pitch, float thr, float yaw)
{
	auto_tasks_send_data.ctrl_flag = ctrl;
	auto_tasks_send_data.roll_or_x = roll;
	auto_tasks_send_data.pitch_or_y = pitch;
	auto_tasks_send_data.thr_z = thr;
	auto_tasks_send_data.yaw = yaw;
	App_Send_Data(0, 0,MY_CTRL_CMD_SET, API_CTRL_REQUEST, (unsigned char*)&auto_tasks_send_data, sizeof(auto_tasks_send_data), NULL, 0, 0);
}
...
void flySquare()
{
        ...
        for(i=0;i<100;i++)
        {
                auto_tasks_send_data_fun(0x48, 2, 0, 0, 0);
                usleep(20000);
        }
        sleep(2);
        //fly to east
        for(i=0;i<100;i++)
        {
                auto_tasks_send_data_fun(0x48, 0, 2, 0, 0);
                usleep(20000);
        }
        sleep(2);
        //fly to south
...
static void Auto_Tasks_Threadfun()
{
	//flySquare();
	flyZsharp();
}

int DJI_Onboard_API_Start_Auto_Tasks(void)
{
	//int ret;
	//pthread_t A_ARR;
	//ret = pthread_create(&A_ARR, 0, Auto_Tasks_Threadfun, NULL);
	//if(ret != 0)
	//{
	//	return -1;
	//}
	Auto_Tasks_Threadfun();
	return 0;
}
...
{
	//read device,baudrate,id,key config from ../output/Config.txt file by evan.gu
        FILE* configFile;
        char* line = NULL;
        size_t len = 0;
        ssize_t read;
